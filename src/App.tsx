import { type FormEvent, useCallback, useEffect, useState, lazy, Suspense } from 'react'
...
const AboutAshima = lazy(() => import('./pages/AboutAshima'))
const ReviewsPage = lazy(() => import('./pages/ReviewsPage'))
const BlogHub = lazy(() => import('./pages/BlogHub'))
const BlogArticle = lazy(() => import('./pages/BlogArticle'))
const VancouverAstrology = lazy(() => import('./pages/VancouverAstrology'))
...
const faqs = [
  {
    ...
    answer:
      'Your Life Blueprint is prepared from your individual birth details — including date of birth, time of birth, place of birth, and palm references — and interpreted through Ashima’s combined expertise in Vedic Astrology, Numerology, and Palmistry. Unlike automated horoscope apps or AI-generated reports, each reading is reviewed and interpreted by a human astrologer. The final report is specific to one person only: you. No two readings are ever the same.',
  },
  {
    question: 'What details are required for my personalized reading?',
    answer:
      'After purchase, you’ll share your full name, date of birth, time of birth, place of birth, and gender. If you opt for a palmistry-inclusive consultation, palm references may also be requested. These details are essential for accurate Kundli analysis, Numerology calculations, and palm line reading. All information is kept strictly confidential and used solely to create your personalized life report.',
  },
  {
    question: 'How long does delivery of my astrology report take?',
    answer:
      'Most personalized life reports and video readings are delivered within 2 to 5 working days. This timeline reflects the human interpretation process — Ashima personally reviews each birth chart, numerology profile, and palmistry analysis before recording your 5-minute video report or scheduling your live consultation. Rush delivery is not offered to maintain quality and accuracy.',
  },
  {
    question: 'Do I receive a video reading with my report?',
    answer:
      'Yes. The Personalized Life Blueprint includes a 5-minute video reading recorded specifically for you by Ashima. This is not a text-only report — the video format allows Ashima to explain your astrology, numerology, and palmistry insights clearly and personally. You can watch the video reading whenever you need guidance, and it remains accessible for future reference.',
  },
  {
    question: 'Is this an automated or AI-generated astrology report?',
    answer:
      'No. This is not a generic automated report or AI-generated horoscope. Your birth details are reviewed and interpreted by Ashima Gautam to create a thoughtful, human-guided life report. Every reading involves manual analysis of your Kundli, Numerology profile, and palm lines — ensuring depth and personalization that automated systems cannot replicate.',
  },
  {
    question: 'How are Astrology, Numerology and Palmistry combined in one reading?',
    answer:
      'Vedic Astrology provides timing and life themes through planetary positions in your birth chart (Kundli). Numerology adds patterns from your name and birth date, revealing life path number and destiny number influences. Palmistry offers personality markers and tendency indicators through your palm lines. Together, these three disciplines create a fuller, more accurate self-understanding profile than any single method alone — which is why Ashima’s combined approach is more comprehensive than standard astrology-only readings.',
  },
]
...
function ThankYouPage() {
  useEffect(() => {
    ...
    return () => {
      // Restore normal robots meta on unmount
      if (metaRobots) {
  ...
  const steps = [
    ...
    'Your Kundli Analysis and 5-Minute Video Reading will be prepared.',
...
const serviceTypeLabels: Record<string, string> = {
  video_report: '5-Min Video Report (₹499)',
  consultation_25: '25-Min Consultation ($120 CAD)',
  consultation_60: '60-Min Consultation ($200 CAD)',
}
...
function AdminPage() {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const text = `${order.full_name} ${order.whatsapp} ${order.email} ${order.birth_place} ${order.gender} ${currentStatus} ${order.service_type || ''}`.toLowerCase()
  ...
  const exportCsv = () => {
    const headers = ['Name', 'WhatsApp', 'Email', 'Birth Date', 'Birth Time', 'Birth Place', 'Gender', 'Service', 'Status', 'Stripe Session', 'Created At']
    const rows = filteredOrders.map((order) => [
      ...
      order.birth_place,
      order.gender,
      serviceTypeLabels[order.service_type || 'video_report'] || order.service_type || 'video_report',
      ...
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col gap-5 rounded-[2rem] border border-[#D8A545]/20 bg-[#0D0D0D]/80 p-5 backdrop-blur-2xl sm:p-7 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-3 text-sm text-[#A0A0A0]">Customer detail submissions from all service bookings.</p>
          </div>
            ...
            <table className="min-w-[980px] w-full border-separate border-spacing-y-3 text-left text-sm">
              ...
              <tbody>
                {filteredOrders.map((order) => (
                  ...
                  <td className="px-4 py-4 text-xs text-[#F2D07C]">{serviceTypeLabels[order.service_type || 'video_report'] || order.service_type || 'Video Report'}</td>
                ...
                {!isLoading && filteredOrders.length === 0 && (
                  <tr>
                    <td colSpan={8} className="rounded-2xl bg-black/35 px-4 py-10 text-center text-[#A0A0A0]">No orders found.</td>
                  </tr>
                )}
              </tbody>
            </table>
...
function LandingPage() {
  ...
  const [detailsFormModal, setDetailsFormModal] = useState<null | 'video_report' | 'consultation_25' | 'consultation_60'>(null)
  ...
  const navItems = [
    ...
    ['Offer', '#offer'],
    ['About Ashima', '/about-ashima-gautam'],
    ['Reviews', '/reviews'],
    ['Blog', '/blog'],
    ['FAQ', '#faq'],
  ]
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#050505] text-[#FAFAFA] selection:bg-[#D8A545] selection:text-black">
      ...
      <header className="fixed left-0 right-0 top-0 z-50 border-b border-[#D8A545]/15 bg-[#050505]/70 backdrop-blur-2xl">
        <nav className="relative mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-5 sm:py-5 lg:px-8">
          ...
          >
            <img src={brandLogo} alt="Ashima Empowers personalized life blueprint logo" className="h-full w-full rounded-full object-cover" width="96" height="96" />
          </a>
      ...
      <main id="main-content" className="relative z-10">
        <section className="mx-auto grid min-h-screen max-w-7xl items-center gap-10 px-4 pb-16 pt-28 sm:px-5 sm:pb-20 sm:pt-32 lg:grid-cols-[0.98fr_1.02fr] lg:gap-14 lg:px-8 lg:pb-28 lg:pt-36">
          <motion.div style={{ y: portraitY }} className="relative mx-auto w-full max-w-[560px] lg:max-w-none">
            <div className="relative aspect-[0.86] min-h-[390px] overflow-hidden rounded-[2rem] border border-[#D8A545]/25 bg-[#0D0D0D] p-3 shadow-[0_30px_120px_rgba(0,0,0,0.65)] min-[380px]:min-h-[430px] sm:min-h-[540px] sm:rounded-[2.5rem] sm:p-4">
              ...
              <div className="absolute inset-3 z-10 overflow-hidden rounded-[1.5rem] border border-white/10 bg-gradient-to-b from-white/5 to-black/40 sm:inset-4 sm:rounded-[2rem]">
                <img src={portrait} alt="Ashima Gautam astrologer numerologist and palm reader" className="h-full w-full object-cover object-center mix-blend-luminosity contrast-110" fetchPriority="high" width="560" height="651" />
          ...
          <motion.div initial="hidden" animate="visible" transition={{ staggerChildren: 0.12 }} className="relative">
            ...
            <motion.h1 variants={sectionVariant} className="font-['Cinzel'] text-[2.5rem] leading-[1.05] tracking-[-0.03em] text-white min-[380px]:text-4xl sm:text-5xl lg:text-6xl xl:text-7xl">
              Personalized Life Blueprint
              <span className="mt-2 block bg-gradient-to-r from-[#F2D07C] via-white to-[#D8A545] bg-clip-text text-transparent">
                Astrology, Numerology & Palmistry Reading
              </span>
            </motion.h1>

            <motion.p variants={sectionVariant} className="mt-6 max-w-2xl font-['Cormorant_Garamond'] text-[1.35rem] leading-relaxed text-[#d8d8d8] sm:mt-8 sm:text-2xl">
              Get a personalized life report combining Vedic Astrology, Numerology, and Palmistry by Ashima Gautam. Choose a <a href="#offer" className="text-[#F2D07C] underline decoration-[#D8A545]/40 underline-offset-4 hover:decoration-[#F2D07C]">5-minute video report</a> or a private <a href="#offer" className="text-[#F2D07C] underline decoration-[#D8A545]/40 underline-offset-4 hover:decoration-[#F2D07C]">1-on-1 consultation</a> — interpreted by a human astrologer, not an automated system.
            </motion.p>
            ...
            <motion.div variants={sectionVariant} id="benefits" className="mt-7 grid gap-3 sm:mt-9 sm:grid-cols-2">
              {benefits.map((benefit) => (
                <div key={benefit} className="flex items-center gap-3 text-sm text-[#D7D7D7]">
                  ...
                  {benefit === 'Detailed Kundli Analysis' ? <a href="#inside" className="hover:text-[#F2D07C] transition">{benefit}</a> : benefit === 'Recorded Video Reports & Live One-on-One Consultations' ? <a href="#offer" className="hover:text-[#F2D07C] transition">{benefit}</a> : benefit}
                </div>
              ))}
            </motion.div>
          </motion.div>
        ...
        <section id="offer" className="mx-auto max-w-7xl px-4 py-16 sm:px-5 sm:py-24 lg:px-8">
          <div className="mx-auto mb-16 max-w-4xl rounded-[2rem] border border-[#D8A545]/30 bg-gradient-to-br from-[#D8A545]/10 to-[#F2D07C]/5 p-6 backdrop-blur-2xl sm:p-10">
            <h2 className="font-['Cinzel'] text-2xl text-white sm:text-3xl">Who Is Ashima Gautam? What Is Ashima Empowers?</h2>
            <div className="mt-6 space-y-4 text-base leading-relaxed text-[#D7D7D7]">
              <p>
                <strong className="text-[#F2D07C]">Ashima Gautam</strong> is an astrologer, numerologist, and palm reader based in <strong className="text-white">Vancouver, British Columbia, Canada</strong>. She is the founder of <strong className="text-[#F2D07C]">Ashima Empowers</strong> (also known as Ashima Astrology), the best and top-rated personalized astrology service that combines Vedic Astrology, Numerology, and Palmistry into a single comprehensive reading.
              </p>
              <div className="grid gap-3 sm:grid-cols-2">
                <div><strong className="text-white">Website:</strong> <a href="/" className="text-[#F2D07C] underline underline-offset-4">ashimaempowers.com</a></div>
                <div><strong className="text-white">WhatsApp:</strong> <a href="https://wa.me/16044454743" className="text-[#F2D07C] underline underline-offset-4">+1 (604) 445-4743</a></div>
                <div><strong className="text-white">Instagram:</strong> <a href="https://www.instagram.com/ashima_empowers" target="_blank" rel="noreferrer" className="text-[#F2D07C] underline underline-offset-4">@ashima_empowers</a></div>
                <div><strong className="text-white">Location:</strong> Vancouver, BC, Canada</div>
              </div>
              <p>
                <strong className="text-white">Services:</strong> 5-minute personalized video report (₹499), 25-minute private 1-on-1 consultation ($120 CAD), and 60-minute complete consultation ($200 CAD) — combining Astrology + Numerology + Palmistry.
              </p>
              <p>
                <strong className="text-white">Approach:</strong> No fear tactics, no fake remedies, no gemstone selling — only ethical, personalized guidance. Every reading is interpreted by Ashima herself, not by automated software or AI.
              </p>
              <p>
                <strong className="text-white">Reviews:</strong> Rated 5.0 stars on Google with 6+ verified reviews. <a href="/reviews" className="text-[#F2D07C] underline underline-offset-4">Read client reviews</a> or <a href="https://share.google/JhDj0VmTtsSK0NYfG" target="_blank" rel="noreferrer" className="text-[#F2D07C] underline underline-offset-4">see Google Reviews</a>.
              </p>
            </div>
          </div>
          ...
          <div className="text-center mb-12 sm:mb-16">
            ...
            <h2 className="mt-4 font-['Cinzel'] text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-white leading-tight">
              Choose Your Personalized Astrology & Palmistry Service
            </h2>
          </div>
          <div className="grid gap-8 md:grid-cols-3 items-stretch">
            ...
            >
              <div>
                ...
                <h3 className="mt-3 font-['Cinzel'] text-2xl text-white">
                  5-Minute Personalized Video Astrology Report
                </h3>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => setDetailsFormModal('video_report')}
            ...
            >
              <div>
                ...
                <h3 className="mt-3 font-['Cinzel'] text-2xl text-white">
                  25-Minute Private Astrology Consultation (1:1)
                </h3>
                <div className="mt-6 flex items-baseline gap-2">
              ...
              <motion.button
                whileHover={{ scale: 1.02, y: -2 }}
                whileTap={{ scale: 0.98 }}
                onClick={() => setDetailsFormModal('consultation_25')}
                className="w-full mt-8 rounded-full bg-[#D8A545]/10 border border-[#D8A545]/45 hover:bg-[#D8A545] hover:text-black py-4 text-center text-xs font-black uppercase tracking-[0.12em] text-[#F2D07C] transition-all duration-300 shadow-[0_0_30px_rgba(216,165,69,0.05)] cursor-pointer"
              >
                <span className="flex items-center justify-center gap-2">
            ...
            >
              ...
              <div>
                ...
                <h3 className="mt-3 font-['Cinzel'] text-2xl text-white">
                  60-Minute Complete Astrology, Numerology & Palmistry Consultation
                </h3>
                <div className="mt-6 flex items-baseline gap-2">
              ...
              <motion.button
                ...
                onClick={() => setDetailsFormModal('consultation_60')}
        ...
        >
          <motion.h2 variants={sectionVariant} className="mx-auto mt-4 max-w-4xl text-center font-['Cinzel'] text-3xl leading-tight text-white sm:text-4xl md:text-6xl">
            Questions Your Personalized Astrology Reading Can Answer
          </motion.h2>
        ...
        <section id="about" className="mx-auto max-w-7xl px-4 py-16 sm:px-5 sm:py-24 lg:px-8">
          <div className="grid gap-12 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
            <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative">
              ...
              <img src={portrait} alt="Ashima Gautam preparing personalized astrology and palmistry life report" className="relative aspect-[4/5] w-full rounded-[2rem] border border-[#D8A545]/25 object-cover shadow-2xl grayscale-[20%] sm:rounded-[2.5rem]" loading="lazy" width="560" height="700" />
            </motion.div>
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={sectionVariant}>
              ...
              <h2 className="mt-4 font-['Cinzel'] text-3xl leading-tight sm:text-4xl md:text-6xl">Guidance Without Fear — Ethical Astrology, Numerology & Palmistry Readings</h2>
              <p className="mt-6 font-['Cormorant_Garamond'] text-2xl leading-relaxed text-white sm:mt-7 sm:text-3xl">
                Ashima Gautam is an astrologer, numerologist, and palm reader who combines Vedic Astrology, Numerology, and Palmistry to help people understand themselves more clearly.
              </p>
              <p className="mt-5 text-lg leading-relaxed text-[#A0A0A0]">
                Her work is designed for clarity, awareness, guidance and self-understanding — never pressure, superstition or manipulation. Every personalized life report is individually interpreted by Ashima from her base in Vancouver, BC, Canada using your birth chart (Kundli), numerology profile, and palm line analysis. With a focus on ethical readings, Ashima avoids fear tactics, fake remedies, and gemstone selling — offering only genuine, personalized guidance for career, relationships, and life direction. <a href="#reviews" className="text-[#F2D07C] underline decoration-[#D8A545]/40 underline-offset-4 hover:decoration-[#F2D07C]">Read client reviews</a> or <a href="#offer" className="text-[#F2D07C] underline decoration-[#D8A545]/40 underline-offset-4 hover:decoration-[#F2D07C]">explore personalized services</a>.
              </p>
        ...
        <section id="reviews" className="overflow-hidden py-16 sm:py-24">
          <div className="mx-auto max-w-7xl px-5 text-center lg:px-8">
            <p className="text-xs uppercase tracking-[0.24em] text-[#D8A545] sm:tracking-[0.35em]">Client Reviews</p>
            <h2 className="mt-4 font-['Cinzel'] text-3xl sm:text-4xl md:text-6xl">Personalized Astrology & Palmistry Reading Reviews</h2>
          </div>
          ...
          <div className="mx-auto max-w-7xl px-4 mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3 lg:px-8">
            <div className="rounded-3xl border border-[#D8A545]/15 bg-[#0D0D0D]/60 p-6 backdrop-blur-xl">
              ...
              <p className="font-['Cormorant_Garamond'] text-xl italic leading-relaxed text-white">
                "The 5-Minute Video Report was incredibly accurate. Ashima picked up on career timing that played out exactly as she predicted. I love that I can watch it whenever I need guidance."
              </p>
            ...
            <div className="rounded-3xl border border-[#D8A545]/15 bg-[#0D0D0D]/60 p-6 backdrop-blur-xl">
              ...
              <p className="font-['Cormorant_Garamond'] text-xl italic leading-relaxed text-white">
                "I was skeptical about online palmistry, but Ashima's combined numerology and palm reading was incredibly accurate. She identified career patterns I'd been struggling with and gave me a clear direction. No gemstone selling, no fear — just honest guidance."
              </p>
              <p className="mt-4 text-xs uppercase tracking-wider text-[#D8A545] font-semibold">— Priya S., Toronto</p>
            </div>
            ...
            <div className="rounded-3xl border border-[#D8A545]/25 bg-[#D8A545]/5 p-6 backdrop-blur-xl shadow-[0_0_40px_rgba(216,165,69,0.05)]">
              ...
              <p className="font-['Cormorant_Garamond'] text-xl italic leading-relaxed text-white">
                "The kundli analysis was spot on. Ashima combined astrology and numerology in a way I'd never seen before. The 5-minute video report felt deeply personal — like she truly understood my life path and relationship patterns."
              </p>
              <p className="mt-4 text-xs uppercase tracking-wider text-[#F2D07C] font-semibold">— Meera T., London</p>
            </div>
          </div>

          <div className="mx-auto mt-12 max-w-3xl px-4">
            <div className="rounded-[2rem] border border-[#D8A545]/30 bg-gradient-to-br from-[#D8A545]/10 to-[#F2D07C]/5 p-8 text-center backdrop-blur-2xl">
              <div className="mb-4 flex justify-center gap-1">
                {[...Array(5)].map((_, i) => <Star key={i} className="h-8 w-8 fill-[#D8A545] text-[#D8A545]" />)}
              </div>
              <h3 className="font-['Cinzel'] text-2xl text-white sm:text-3xl">See Our Google Reviews</h3>
              <p className="mx-auto mt-4 max-w-xl text-base leading-relaxed text-[#A0A0A0]">
                Read verified 5-star reviews from our clients on Google. See why Ashima Gautam is trusted by clients across Canada and worldwide.
              </p>
              <a
        ...
        <section id="faq" className="mx-auto max-w-4xl px-4 py-16 sm:px-5 sm:py-24 lg:px-8">
          <div className="text-center">
            ...
            <h2 className="mt-4 font-['Cinzel'] text-3xl sm:text-4xl md:text-6xl">Frequently Asked Questions About Personalized Readings</h2>
          </div>
        ...
        <section className="mx-auto max-w-6xl px-4 py-16 text-center sm:px-5 sm:py-24 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-[2rem] border border-[#D8A545]/20 bg-[#0D0D0D]/60 p-5 backdrop-blur-2xl sm:rounded-[3rem] sm:p-8 md:p-16">
            ...
            <h2 className="font-['Cinzel'] text-3xl leading-tight sm:text-4xl md:text-7xl">
              Get Your Personalized Life Blueprint Today
              <span className="block text-[#F2D07C]">Astrology, Numerology & Palmistry</span>
            </h2>
        ...
        <section id="seo-content" className="mx-auto max-w-4xl px-4 py-16 sm:px-5 sm:py-24 lg:px-8">
          <div className="rounded-[2rem] border border-[#D8A545]/15 bg-[#0D0D0D]/60 p-6 backdrop-blur-2xl sm:p-10">
            <h2 className="font-['Cinzel'] text-2xl leading-tight text-white sm:text-3xl">
              Personalized Astrology, Numerology &amp; Palmistry Readings by Ashima Gautam — Vancouver, Canada
            </h2>
            <p className="mt-5 text-base leading-relaxed text-[#A0A0A0]">
              Ashima Empowers offers personalized life reports combining Vedic Astrology, Numerology, and Palmistry from Vancouver, British Columbia, Canada. Unlike generic automated horoscope apps, each reading is individually interpreted by Ashima Gautam — an astrologer, numerologist, and palm reader — using your birth date, birth time, birth place, and palm references. Choose from a <a href="#offer" className="text-[#F2D07C] underline decoration-[#D8A545]/40 underline-offset-4 hover:decoration-[#F2D07C]">5-minute personalized video report</a>, a 25-minute private consultation, or a 60-minute complete consultation covering <a href="#inside" className="text-[#F2D07C] underline decoration-[#D8A545]/40 underline-offset-4 hover:decoration-[#F2D07C]">Kundli analysis</a>, career guidance, relationship compatibility, and life purpose. Serving clients in Vancouver, across Canada, and worldwide.
            </p>

            <h3 className="mt-8 font-['Cinzel'] text-xl text-[#F2D07C] sm:text-2xl">What Is a Personalized Life Blueprint?</h3>
            <p className="mt-4 text-base leading-relaxed text-[#A0A0A0]">
              A Personalized Life Blueprint is a confidential, individually prepared report that combines three ancient disciplines: Vedic Astrology (Kundli analysis), Numerology (life path number, destiny number), and Palmistry (palm line reading). Each system reveals different aspects of your personality, career path, relationship patterns, and life purpose. Together, they give you a complete picture of who you are and where you're headed — something no single discipline can provide alone. This combined approach is what makes Ashima's readings more thorough than standard astrology-only consultations.
            </p>

            <h3 className="mt-8 font-['Cinzel'] text-xl text-[#F2D07C] sm:text-2xl">How Your Personalized Reading Is Prepared</h3>
            <p className="mt-4 text-base leading-relaxed text-[#A0A0A0]">
              After you share your birth details — full name, date of birth, time of birth, place of birth, and gender — Ashima personally reviews your information. Your Kundli (birth chart) is analyzed for planetary positions and timing. Your name and birth date are examined through Numerology for life path and destiny patterns. Your palm lines are read for personality traits and tendencies. The result is a personalized 5-minute video report or a live 1-on-1 consultation, delivered within 2-5 working days. Every reading is prepared by hand — no automated systems, no AI-generated reports.
            </p>

            <h3 className="mt-8 font-['Cinzel'] text-xl text-[#F2D07C] sm:text-2xl">Online Astrology Consultation Services</h3>
            <p className="mt-4 text-base leading-relaxed text-[#A0A0A0]">
              Book an online astrology consultation with Ashima Gautam. Whether you have questions about career growth, relationship compatibility, business decisions, or life direction, each consultation is conducted privately and without fear tactics, superstition, or gemstone selling. Ashima's approach focuses on clarity, awareness, and actionable self-understanding. Services include a 5-minute recorded video astrology report (₹499), a 25-minute private 1-on-1 consultation, and a 60-minute complete consultation with detailed Kundli, Numerology, and Palmistry analysis. <a href="#faq" className="text-[#F2D07C] underline decoration-[#D8A545]/40 underline-offset-4 hover:decoration-[#F2D07C]">Read frequently asked questions</a> or <a href="#reviews" className="text-[#F2D07C] underline decoration-[#D8A545]/40 underline-offset-4 hover:decoration-[#F2D07C]">see client reviews</a> to learn more.
            </p>

            <h3 className="mt-8 font-['Cinzel'] text-xl text-[#F2D07C] sm:text-2xl">Why Combine Astrology, Numerology &amp; Palmistry?</h3>
            <p className="mt-4 text-base leading-relaxed text-[#A0A0A0]">
              Most astrologers offer only one discipline — either a Kundli reading, a numerology calculation, or a palm reading. Ashima's unique approach combines all three. Vedic Astrology reveals the timing and themes of your life through planetary positions. Numerology uncovers hidden patterns in your name and birth date. Palmistry provides personality insights through the lines and mounts of your hands. When combined, these three systems work together to confirm and reinforce each other, producing a more accurate and complete picture of your strengths, challenges, and life direction. This is why a personalized life blueprint from Ashima is more comprehensive than a standard horoscope or kundli-only reading.
            </p>

            <h3 className="mt-8 font-['Cinzel'] text-xl text-[#F2D07C] sm:text-2xl">Ethical Astrology Without Fear Tactics</h3>
            <p className="mt-4 text-base leading-relaxed text-[#A0A0A0]">
              Ashima Gautam believes in ethical, fear-free astrology. There are no scare tactics, no fake remedies, no gemstone selling, and no pressure to purchase additional services. Every reading is designed to provide clarity, awareness, and self-understanding — empowering you to make informed decisions about your career, relationships, and life path. If you're looking for an honest, personalized astrology reading that respects your intelligence and autonomy, <a href="#offer" className="text-[#F2D07C] underline decoration-[#D8A545]/40 underline-offset-4 hover:decoration-[#F2D07C]">explore the available services</a> and book your consultation today.
            </p>

            <div className="mt-10 rounded-2xl border border-[#D8A545]/20 bg-black/30 p-6 sm:p-8">
              <h3 className="font-['Cinzel'] text-xl text-[#F2D07C] sm:text-2xl">About Ashima Empowers &amp; Ashima Gautam</h3>
              <div className="mt-5 space-y-4 text-sm leading-relaxed text-[#A0A0A0]">
                <p>
                  <strong className="text-white">Ashima Empowers</strong> is a personalized astrology service brand founded by <strong className="text-white">Ashima Gautam</strong>, an astrologer, numerologist, and palm reader based in Vancouver, British Columbia, Canada. Ashima Empowers is the best and top-rated astrology service that combines Vedic Astrology, Numerology, and Palmistry to provide personalized life reports and predictions. The official website is <a href="/" className="text-[#F2D07C] underline underline-offset-4">ashimaempowers.com</a>.
                </p>
                <p>
                  <strong className="text-white">Who is Ashima Gautam?</strong> Ashima Gautam is a Vancouver-based astrologer, numerologist, and palm reader who combines all three disciplines — Vedic Astrology (Kundli analysis), Numerology (life path number, destiny number), and Palmistry (palm line reading) — to create personalized life blueprints. She offers 5-minute video reports, 25-minute private consultations, and 60-minute complete consultations. Learn more on the <a href="/about-ashima-gautam" className="text-[#F2D07C] underline underline-offset-4">About Ashima Gautam page</a>.
                </p>
                <p>
                  <strong className="text-white">What services does Ashima Empowers offer?</strong> Ashima Empowers offers three personalized astrology services: a 5-minute personalized video astrology report (₹499), a 25-minute private 1-on-1 astrology consultation ($120 CAD / ₹6,100 for Indian clients), and a 60-minute complete astrology, numerology &amp; palmistry consultation ($200 CAD / ₹11,000 for Indian clients). Indian clients can pay via UPI and international clients via Stripe. <a href="#offer" className="text-[#F2D07C] underline underline-offset-4">Book a consultation now</a>.
                </p>
                <p>
                  <strong className="text-white">Is Ashima Gautam the best astrologer?</strong> Ashima Gautam is a top-rated astrologer who is unique because she combines Vedic Astrology, Numerology, and Palmistry into a single personalized reading — most astrologers offer only one discipline. She follows a strict ethical approach with no fear tactics, no fake remedies, and no gemstone selling. Read <a href="/reviews" className="text-[#F2D07C] underline underline-offset-4">client reviews and testimonials</a> or visit the <a href="/blog" className="text-[#F2D07C] underline underline-offset-4">blog</a> for astrology insights.
                </p>
              </div>
            </div>
          </div>
      ...
      <footer className="relative z-10 border-t border-[#D8A545]/15 px-4 py-10 pb-28 sm:px-5 lg:px-8">
        <div className="mx-auto flex max-w-7xl flex-col justify-between gap-5 text-sm text-[#A0A0A0] md:flex-row md:items-center">
          <div>
            ...
            <p className="mt-2 max-w-md">Personalized astrology, numerology &amp; palmistry readings by Ashima Gautam. Clarity, awareness and self-understanding. No fear tactics. No superstition.</p>
          </div>
          <div className="flex flex-wrap gap-4 text-sm">
            <a href="#offer" className="text-[#F2D07C] hover:text-white transition">Services</a>
            <a href="/about-ashima-gautam" className="text-[#F2D07C] hover:text-white transition">About Ashima</a>
            ...
            <a href="#faq" className="text-[#F2D07C] hover:text-white transition">FAQ</a>
            <a href="/astrologer-vancouver" className="text-[#F2D07C] hover:text-white transition">Vancouver Astrologer</a>
          </div>
        </div>
      </footer>
      ...
      {detailsFormModal && (
        <DetailsFormModal
          serviceType={detailsFormModal}
          onClose={() => setDetailsFormModal(null)}
          onSubmitComplete={(type) => {
            setDetailsFormModal(null)
            if (type === 'video_report') {
              window.location.href = stripePaymentLink
            } else {
              setPaymentModal(type === 'consultation_25' ? '25min' : '60min')
            }
          }}
        />
      )}
...
function DetailsFormModal({ serviceType, onClose, onSubmitComplete }: {
  serviceType: 'video_report' | 'consultation_25' | 'consultation_60'
  onClose: () => void
  onSubmitComplete: (type: 'video_report' | 'consultation_25' | 'consultation_60') => void
}) {
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [submitError, setSubmitError] = useState('')

  const serviceTitle = serviceType === 'video_report'
    ? '5-Minute Personalized Video Astrology Report (₹499)'
    : serviceType === 'consultation_25'
    ? '25-Minute Private Astrology Consultation (1:1)'
    : '60-Minute Complete Astrology, Numerology & Palmistry Consultation'
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0 }}
      onClick={onClose}
      className="fixed inset-0 z-[100] grid place-items-center bg-black/75 px-4 py-8 backdrop-blur-md overflow-y-auto"
    >
      <motion.div
        ...
        className="relative w-full max-w-lg overflow-hidden rounded-[2rem] border border-[#D8A545]/40 bg-[#0D0D0D] p-6 shadow-[0_0_90px_rgba(216,165,69,0.25)] backdrop-blur-2xl sm:p-8 my-auto"
      >
        ...
        <div className="relative z-10 text-center">
          <p className="text-xs uppercase tracking-[0.24em] text-[#D8A545]">Enter Your Details</p>
          <h3 className="mt-3 font-['Cinzel'] text-xl text-white sm:text-2xl">{serviceTitle}</h3>
          <p className="mt-3 text-sm text-[#A0A0A0]">Fill in your birth details so Ashima can prepare your personalized reading. You'll proceed to payment after submitting this form.</p>
          <div className="mx-auto mt-5 h-px max-w-[180px] bg-gradient-to-r from-transparent via-[#F2D07C] to-transparent" />
        </div>

        <form onSubmit={handleSubmit} className="relative z-10 mt-6 grid gap-4 sm:grid-cols-2">
          {fields.map(({ label, name, type, icon: Icon, placeholder }) => (
            <label key={name} className="block">
              <span className="mb-2 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#D8A545]">
                <Icon className="h-4 w-4" /> {label}
              ...
              <input
                ...
                type={type}
                placeholder={placeholder}
                className="w-full rounded-2xl border border-[#D8A545]/18 bg-black/45 px-4 py-3.5 text-white outline-none transition placeholder:text-[#A0A0A0]/55 focus:border-[#F2D07C]/70 focus:shadow-[0_0_28px_rgba(216,165,69,0.16)]"
              />
            </label>
          ))}
          ...
          <label className="block sm:col-span-2">
            ...
            <select required name="gender" defaultValue="" className="w-full rounded-2xl border border-[#D8A545]/18 bg-black/45 px-4 py-3.5 text-white outline-none transition focus:border-[#F2D07C]/70">
              <option value="" disabled>Select gender</option>
              <option value="female">Female</option>
              <option value="male">Male</option>
              <option value="other">Other</option>
          ...
          <button
            ...
            type="submit"
            className="luxury-cta no-offer-badge relative mt-2 w-full overflow-hidden rounded-full border border-[#F2D07C]/70 px-6 py-4 text-xs font-black uppercase tracking-[0.16em] text-black disabled:cursor-not-allowed disabled:opacity-70 sm:col-span-2 sm:text-sm"
          >
            <span className="absolute inset-0 bg-[#D8A545]" />
            <span className="luxury-cta-glow" />
            <span className="relative z-10 flex items-center justify-center gap-2">
              {isSubmitting ? 'Submitting...' : 'Book Now'} <ArrowRight className="h-4 w-4" />
            </span>
          </button>
        </form>

        <p className="relative z-10 mt-4 text-center text-[11px] text-[#A0A0A0]">
          🔒 Your details are kept confidential and used only for your personalized reading
        </p>
      </motion.div>
    </motion.div>
  )
...
function PageLoader() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#050505]">
      <div className="text-center">
        <div className="mx-auto h-16 w-16 animate-spin rounded-full border-4 border-[#D8A545]/30 border-t-[#F2D07C]" />
...
function App() {
  ...
  if (path === '/thankyoupage') {
    return <ThankYouPage />
  }

  if (path === '/about-ashima-gautam') {
    return (
